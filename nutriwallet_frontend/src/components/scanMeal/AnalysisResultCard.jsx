...
import HealthAlertBanner from "../common/HealthAlertBanner";
...
export default function AnalysisResultCard({
    ...
    return (
      <section className="flex min-h-[560px] items-center justify-center rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          ...
          <h2 className="mt-5 text-lg font-semibold text-slate-950 dark:text-white">Chưa có kết quả phân tích</h2>
          ...
          {healthProfile?.healthConsentGiven && (
            <p className="mt-3 flex items-center justify-center gap-1.5 text-xs text-emerald-600 dark:text-emerald-400">
              ...
              Cảnh báo sức khoẻ cá nhân đã được kích hoạt
            </p>
          )}
        </div>
      </section>
  ...
  const handleStartEdit = () => { setDraftResult(result); setIsEditing(true); };
  const handleCancelEdit = () => { setDraftResult(result); setIsEditing(false); };
  const handleSaveEdit = () => { onUpdateResult?.(draftResult); setIsEditing(false); };

  const updateFoodName = (value) => setDraftResult((prev) => ({ ...prev, foodName: value }));
  const updateMealType = (value) => setDraftResult((prev) => ({ ...prev, mealType: value || null }));
  const updateEstimatedPrice = (value) => setDraftResult((prev) => ({ ...prev, estimatedPrice: Number(value) }));
  const updateNutrition = (field, value) => setDraftResult((prev) => ({ ...prev, nutrition: { ...prev.nutrition, [field]: Number(value) } }));
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Sparkles className="text-emerald-600 dark:text-emerald-400" size={22} aria-hidden="true" />
          <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Kết quả phân tích AI</h2>
        </div>
      ...
      {healthAlerts.length > 0 && (
        <div className="mt-4">
          <HealthAlertBanner alerts={healthAlerts} variant="compact" />
        </div>
      )}

      {isEditing ? (
        <div className="mt-5">
          <label className="text-sm font-medium text-slate-600 dark:text-slate-400" htmlFor="edit-food-name">Tên món ăn</label>
          <input
            id="edit-food-name"
        ...
        <>
          <h3 className="mt-5 text-2xl font-bold text-slate-950 dark:text-white">{displayResult.foodName}</h3>
      ...
      {isEditing ? (
        <div className="mt-5 grid grid-cols-2 gap-3">
          <EditNumberField label="Calories" unit="kcal" value={draftResult.nutrition.calories} onChange={(value) => updateNutrition("calories", value)} />
          <EditNumberField label="Protein" unit="g" value={draftResult.nutrition.protein} onChange={(value) => updateNutrition("protein", value)} />
          <EditNumberField label="Carbs" unit="g" value={draftResult.nutrition.carbs} onChange={(value) => updateNutrition("carbs", value)} />
          <EditNumberField label="Fat" unit="g" value={draftResult.nutrition.fat} onChange={(value) => updateNutrition("fat", value)} />
          <EditNumberField label="Giá ước tính" unit={draftResult.currency} value={draftResult.estimatedPrice} onChange={updateEstimatedPrice} className="col-span-2" />
          <div className="col-span-2">
            <label className="text-sm font-medium text-slate-600 dark:text-slate-400" htmlFor="edit-meal-type">Loại bữa ăn</label>
            <select id="edit-meal-type" value={draftResult.mealType || ""} onChange={(e) => updateMealType(e.target.value)}
              className="mt-2 h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm font-semibold text-slate-900 outline-none transition focus:border-emerald-500 dark:border-slate-800 dark:bg-slate-800 dark:text-white">
              <option value="">Không xác định</option>
              {MEAL_TYPE_OPTIONS.map((type) => <option key={type} value={type}>{MEAL_TYPE_META[type]?.label ?? type}</option>)}
            </select>
          </div>
        </div>
      ) : (
        <div className="mt-5 space-y-3">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <NutritionCard icon={Flame} label="Calories" value={formatRangeValue(displayResult.nutrition.calories)} unit="kcal" color="text-orange-500" compact />
            <NutritionCard icon={Beef} label="Protein" value={formatRangeValue(displayResult.nutrition.protein)} unit="g" color="text-emerald-600 dark:text-emerald-400" compact />
            <NutritionCard icon={Wheat} label="Carbs" value={formatRangeValue(displayResult.nutrition.carbs)} unit="g" color="text-amber-500" compact />
            <NutritionCard icon={Droplet} label="Fat" value={formatRangeValue(displayResult.nutrition.fat)} unit="g" color="text-red-500" compact />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <NutritionCard icon={Wallet} label="Giá ước tính" value={(displayResult.estimatedPrice ?? 0).toLocaleString("vi-VN")} unit={displayResult.currency} color="text-emerald-600 dark:text-emerald-400" />
            <NutritionCard icon={MEAL_TYPE_META[displayResult.mealType]?.icon || Coffee} label="Loại bữa ăn" value={MEAL_TYPE_META[displayResult.mealType]?.label || "Chưa xác định"} unit="" color="text-sky-600 dark:text-sky-400" />
          </div>
        </div>
      )}
      ...
      <div className="mt-7">
        <h4 className="mb-3 flex items-center gap-2 font-semibold text-slate-950 dark:text-white">
          <Leaf size={18} className="text-emerald-600 dark:text-emerald-400" aria-hidden="true" />
        ...
        <div className="divide-y divide-slate-100 text-sm dark:divide-slate-800">
          ...
          <DetailRow label="Độ tin cậy" value={`${displayResult.ai.confidence ?? 0}%`} />
          <DetailRow label="Loại bữa ăn" value={MEAL_TYPE_META[displayResult.mealType]?.label || "Chưa xác định"} />
          <DetailRow label="Giá ước tính" value={`${(displayResult.estimatedPrice ?? 0).toLocaleString("vi-VN")} ${displayResult.currency}`} />
      ...
      {isEditing ? (
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button type="button" onClick={handleCancelEdit}
            ...
            <X size={18} aria-hidden="true" /> Hủy
          </button>
          <button type="button" onClick={handleSaveEdit}
            ...
            <Save size={18} aria-hidden="true" /> Lưu chỉnh sửa
          </button>
        </div>
      ) : (
        <div className="mt-6 space-y-3">
          ...
          <button type="button" onClick={handleStartEdit}
            ...
            <Pencil size={18} aria-hidden="true" /> Chỉnh sửa kết quả
          </button>
          <button type="button" onClick={() => setShowReportModal(true)}
            ...
            <AlertTriangle size={18} aria-hidden="true" /> Báo lỗi nhận diện AI
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-[28px] border border-slate-200 bg-white p-6 shadow-2xl space-y-6 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider" htmlFor="report-reason">Lý do lỗi</label>
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider" htmlFor="report-desc">Mô tả chi tiết lỗi</label>
                <textarea id="report-desc" value={reportDescription} onChange={(e) => setReportDescription(e.target.value)}
