  ...
  return (
    <button
      ...
      className={`group flex w-full cursor-pointer items-center gap-2.5 rounded-2xl px-3 py-2.5 text-left text-[15px] font-medium transition-all duration-200 ${
        active
          ? "bg-emerald-50 text-emerald-600 shadow-sm shadow-emerald-100/60"
...
export default function Sidebar({ mobile = false, onClose }) {
  const wrapperClass = mobile
    ? "relative flex h-full w-[248px] flex-col bg-white px-3 py-4"
    : "relative hidden min-h-screen w-[224px] shrink-0 flex-col border-r border-slate-200 bg-white px-3 py-4 xl:flex";
  ...
  return (
    <aside className={wrapperClass}>
      ...
      <nav className="space-y-1.5">
        <SidebarItem icon={LayoutDashboard} label="Tổng quan" active onClick={onClose} />
        ...
        <SidebarItem icon={ReceiptText} label="Lịch sử chi tiêu" onClick={onClose} />
        <SidebarItem icon={User} label="Hồ sơ" onClick={onClose} />
        <SidebarItem icon={Settings} label="Cài đặt" onClick={onClose} />
      </nav>

      <div className="pointer-events-none absolute inset-x-4 bottom-24 h-24 rounded-full bg-[radial-gradient(circle,rgba(16,185,129,0.06),transparent_70%)] blur-2xl" />

      <div className="relative mt-auto rounded-3xl border border-slate-200 bg-white p-3.5 shadow-sm shadow-slate-200/35">
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 flex items-center gap-1 text-xs font-medium text-emerald-600">
              ...
              {userInfo.emailVerified ? "Email đã xác minh" : "Email chưa xác minh"}
            </p>
          </div>
        </div>

        <div className="mt-3 flex items-center gap-2 border-t border-slate-100 pt-2.5 text-xs text-slate-500">
          <MessageCircle size={14} strokeWidth={1.9} className="text-blue-500" />
